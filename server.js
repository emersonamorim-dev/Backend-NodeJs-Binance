...
app.get("/asset/:symbol", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Erro ao obter informações do ativo ${symbol}` });
  }
});
...
app.post("/buy", async (req, res) => {
  ...
  if (!symbol || !quantity || !price) {
    return res
      .status(400)
      .json({ error: "Símbolo, quantidade e preço são obrigatórios" });
  }
  try {
    ...
    res
      .status(500)
      .json({ error: `Erro ao criar ordem de compra para o ativo ${symbol}` });
  }
});
...
app.post("/sell", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Erro ao criar ordem de venda para o ativo ${symbol}` });
  }
});
